...
M = {
  ...
  autosnippet(
    { trig = '([acCer])ref', name = '(acC|eq)?ref', dscr = 'add a reference (with autoref, cref, eqref)', trigEngine = 'pattern', hidden = true },
    fmta(
      [[
    \<>ref{<>:<>}<>
    ]],
...
for k, v in pairs(single_command_specs) do
  table.insert(
    ...
    single_command_snippet(vim.tbl_deep_extend('keep', { trig = k }, v.context), v.command, v.opt or { condition = tex.in_text }, v.ext or {})
  )
end
